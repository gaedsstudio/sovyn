...
def list_sessions(store: Store) -> tuple[SessionRecord, ...]:
    with store.connect() as connection:
        rows = connection.execute(
            "SELECT id, request, result, tool_calls, duration_seconds FROM sessions ORDER BY id DESC"
        ).fetchall()
    return tuple(SessionRecord(int(row[0]), str(row[1]), str(row[2]), int(row[3]), float(row[4])) for row in rows)
